...
A QP is sampled per step (or several distinct QPs when ``train.qp_per_step`` is
greater than one) and mapped to a proxy quality; its normalised value is fed to
the preprocessor's FiLM so one model spans the whole rate range. The default
objective has no MSE-to-source term (it fought compression).
...
def _proxy_name(cfg: dict) -> str:
    ...
    if kind == "ste":
        name = cfg.get("codec", {}).get("ste_codec", "h265")
        if isinstance(name, (list, tuple)):
            name = "-".join(map(str, name))
        return f"ste-{str(name).lower()}"
    return kind
