...
class VirtualCodec(nn.Module):
    ...
    def forward(self, x: torch.Tensor, quality: int):
        # Respect module mode: train() uses soft/annealed quantisation, while
        # eval() uses hard rounding for a deterministic validation signal.
        return self._code(x, quality, training=self.training)
